...
typedef struct { block rd_key[11]; unsigned int rounds; } AES_KEY1;

template<int NumKeys>
static inline void ks_rounds(AES_KEY1 * keys, block con, block con3, block mask, int r) {
 ...
 */
template<int NumKeys>
static inline void AES_opt_key_schedule(block* user_key, AES_KEY1 *keys) {
...
static inline void ParaEnc(block *blks, AES_KEY1 *keys) {
	block * first = blks;
